...
function OSProfiler() {
  const [results, setResults] = useState({ crossOriginIsolated: { status: 'loading' }, sharedArrayBuffer: { status: 'loading' }, webAssembly: { status: 'loading' }, isSecureContext: { status: 'loading' }, webGL: { status: 'loading' }, webSocket: { status: 'loading' }, indexedDB: { status: 'loading' }, serviceWorker: { status: 'loading' }, userAgent: { status: 'info', message: navigator.userAgent } });
...
function CLI({ appRootRef }) {
  const [history, setHistory] = useState([{ command: 'welcome', output: 'CLI initialized. Type `help` for a list of commands.' }]);
