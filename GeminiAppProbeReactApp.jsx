    ...
    React.useEffect(() => {
        ...
        Object.keys(originalConsole).forEach(methodName => {
          console[methodName] = (...args) => {
            ...
            setConsoleOutput(prev => [{ level: methodName, message, timestamp: new Date().toISOString() }, ...prev].slice(0, 100));
...
window.GeminiAppProbeReactAppApp = function App() {
        ...
        try {
            switch (testName) {
                ...
                case 'reactFiber':
                    ...
                    if (!fiberKey) {
                        throw new Error("Could not find React Fiber key on the event target.");
                    }
                    
                    const fiberNode = targetElement[fiberKey];

                    if (!fiberNode) {
                        throw new Error("React Fiber node is null or undefined.");
                    }

                    const ownerName = fiberNode.return?.stateNode?.constructor?.name || fiberNode.return?.type?.name || 'Unknown';
                    const props = fiberNode.memoizedProps || {};
                    result = { data: { owner: ownerName, props: Object.keys(props) }, status: 'success'};
                    break;
            ...
            result = { data: e.message, status: 'error' };
        }
    ...
    const handleGeminiSubmit = async (promptToSubmit, isPartOfSuite = false, includeContext = false) => {
        ...
        if (includeContext) {
            const analysisContext = { introspection: introspectionResult, apiTests, firestore: firestoreState };
    ...
    const runAllTestsAndDownload = async () => {
        ...
        const allApiTestKeys = ['fetch', 'popup', 'localStorage', 'sessionStorage', 'indexedDB', 'permissions', 'webWorker', 'webSocket', 'webgl', 'webRtc', 'fileSystemApi', 'mediaDevices', 'gapi_load', 'firebase_auth', 'postMessage', 'wasm'];
        ...
        if (includeGeminiAnalysis) {
            ...
            geminiAnalysisResult = await handleGeminiSubmit(analysisPrompt, true);
        }
        ...
        setConsoleOutput(currentConsoleLog => {
            const fullLog = {
                ...
                // Create a reversed copy for the log file, leaving the state untouched.
                consoleLog: [...currentConsoleLog].reverse() 
            };
            downloadJson(fullLog, 'comprehensive_prober_log.json');
            // Return the original state without mutation.
    ...
    const TestButtonCard = ({ testName, testKey, icon, color = 'blue', onClick }) => {
        const colorClasses = {
        ...
        return (
            <div className="space-y-2">
                <button onClick={onClick || (() => runApiTest(testKey))} disabled={isLoading || isSuiteRunning} className={`w-full p-2 rounded text-left flex items-center space-x-2 transition disabled:bg-slate-500 text-white font-semibold ${colorClasses[color]}`}>
    ...
    const tabs = {
        environment: { label: 'Environment', icon: <CodeIcon/>, content: ( 
            <div>
        ...
        gemini: { label: 'Gemini API', icon: <BrainCircuitIcon/>, content: ( 
            <div className="space-y-4">
                 <textarea value={geminiPrompt} onChange={e => setGeminiPrompt(e.target.value)} rows="4" className="w-full p-2 bg-slate-700 rounded-md border border-slate-600 font-mono text-sm" />
        ...
        browser_apis: { label: 'Browser APIs', icon: <GlobeIcon/>, content: (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        advanced_probes: { label: 'Advanced Probes', icon: <TestTubeIcon/>, content: (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <TestButtonCard testName="Test `WASM`" testKey="wasm" color="purple" icon={<CodeIcon />}/>
            </div>
        )},
         google_services: { label: 'Google Services', icon: <DatabaseIcon/>, content: (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        security_probes: { label: 'Security & Egress', icon: <ShieldIcon/>, content: (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 ...
                 <div className="space-y-4 p-4 bg-slate-800/50 rounded-lg">
                    ...
                    <TestButtonCard testName="Test `React Fiber`" testKey="reactFiber" color="red" icon={<CodeIcon />} onClick={(e) => runApiTest('reactFiber', { event: e })} />
                    <TestButtonCard testName="Test `Direct Tool Use`" testKey="directToolUse" color="red" icon={<BrainCircuitIcon />} onClick={() => handleGeminiSubmit("Use the google_search tool to find out what the weather is in Chico, CA.")} />
                </div>
